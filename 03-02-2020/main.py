...
half_life = pygame.image.load('1_life.png').convert_alpha()
one_life = pygame.image.load('2_life.png').convert_alpha()
one_life_half = pygame.image.load('3_life.png').convert_alpha()
two_life = pygame.image.load('4_life.png').convert_alpha()
two_life_half = pygame.image.load('5_life.png').convert_alpha()
three_life = pygame.image.load('full_life.png').convert_alpha()
full_energy = pygame.image.load('full_energy.png').convert_alpha()
five_energy = pygame.image.load('5_energy.png').convert_alpha()
four_energy = pygame.image.load('4_energy.png').convert_alpha()
three_energy = pygame.image.load('3_energy.png').convert_alpha()
two_energy = pygame.image.load('2_energy.png').convert_alpha()
one_energy = pygame.image.load('1_energy.png').convert_alpha()
zero_energy = pygame.image.load('0_energy.png').convert_alpha()
laser_bullet = pygame.image.load('laser_player.png').convert_alpha()
bonus_life_image = pygame.image.load('life_as_bonus.png').convert_alpha()
...
def initialize():
    global background_y, shooting_enemy_speed, tick_to_damage_boss, shooting_enemies_list, bullets_list, \
            ...
            fighting_boss, boss_present, laser_as_bullet_istance_one, life_as_bonus_is_active_bool, \
            life_as_bonus_istance_one, life_as_bonus_taken_tick, shield_as_bonus_istance_one, \
            ...
            boss_list, boss_list_removed, boss_missile_list, boss_missile_removed, boss_bullets_list, boss_bullets_removed, boss_spawned, \
            temp_boss_life, hash_map_boss_life, boss_defeated, laser_bool, key, mouse, temp, running, change_level_bool, change_level_running

    background_y, shooting_enemy_speed, tick_to_damage_boss, boss_life, score, life_as_bonus_taken_tick, shield_bonus_taken_tick, tick_to_animate_shield, tick_immune_after_shield_boss = 0, 2, 0, 20, 0, 0, 0, 1, 0
    life_as_bonus_is_active_bool, pause_bool, fighting_boss, boss_present, play_again_gameover, shield_as_bonus_is_active_bool, shield_as_bonus_taken = False, False, False, False, False, False, False
    stay_menu = True
    laser_bool, change_level_running = False, False
    ...
    temp_boss_life = 100
    hash_map_boss_life = 600
    bullets_list, shooting_enemies_list, enemy_bullets_list, kamikaze_enemy_list, boss_list, boss_missile_list, boss_bullets_list = [], [], [], [], [], [], []
    bullets_list_removed, shooting_enemies_list_removed, enemy_bullets_list_removed, kamikaze_enemy_removed, boss_list_removed, boss_missile_removed, boss_bullets_removed = [], [], [], [], [], [], []
...
def drawBackground():
    global background_y, change_level_bool
    if not change_level_bool:
...
def deleteCollision():
    global shooting_enemies_list, bullets_list, enemy_bullets_list, kamikaze_enemy_list, explosion_list, boss_missile_list, boss_bullets_list, boss_list
    ...
    boss_bullets_list = [boss_bullet for boss_bullet in boss_bullets_list if boss_bullet not in boss_bullets_removed]
    boss_list = [bosss for bosss in boss_list if bosss not in boss_list_removed]
...
def drawLaser():
    if laser_bool and player_one_istance.energy > 0:
...
def drawBossHealthbar():
    global boss_list, temp_boss_life, hash_map_boss_life
    if boss_present:
        for bosss in boss_list:
            if temp_boss_life > bosss.life:
                hash_map_boss_life -= 6
                temp_boss_life = bosss.life
            bar_length = min(abs(hash_map_boss_life + 100 - 100), display_width - 200)
            bar_percentage = bar_length / (display_width - 100)
            h, s, v = 0.33 * bar_percentage, 1, 1
            r, g, b = colorsys.hsv_to_rgb(h, s, v)
            colour = [int(255 * r), int(255 * g), int(255 * b)]
            pygame.draw.line(screen, (57,62,68), [100, display_height - 50], [700, display_height - 50], 20)
            pygame.draw.line(screen, colour, [100, display_height - 50], [100 + bar_length, display_height - 50], 20)
...
def drawEnergy():
    if player_one_istance.energy == 6:
        screen.blit( full_energy, (display_width - 130, 45) )
    if player_one_istance.energy == 5:
        screen.blit( five_energy, (display_width - 130, 45) )
    if player_one_istance.energy == 4:
        screen.blit( four_energy, (display_width - 130, 45))
    if player_one_istance.energy == 3:
        screen.blit( three_energy, (display_width - 130, 45) )
    if player_one_istance.energy == 2:
        screen.blit( two_energy, (display_width - 130, 45) )
    if player_one_istance.energy == 1:
        screen.blit( one_energy, (display_width - 130, 45) )
    if player_one_istance.energy == 0:
        screen.blit( zero_energy, (display_width - 130, 45) )
...
def checkBoss():
    ...
    if score == 50:   #condition to active the boss
        boss_present = True
        fighting_boss = True

def checkLifeBonus():
    global score, life_as_bonus_is_active_bool, boss_defeated
    if not life_as_bonus_is_active_bool and player_one_istance.life < 6 and boss_defeated:   #condition to active the bonus
        life_as_bonus_is_active_bool = True
        boss_defeated = False
    if not life_as_bonus_is_active_bool and player_one_istance.life < 6 and player_one_istance.life == 1:
        life_as_bonus_is_active_bool = True
...
class EnergyShield:
    ...
    def collideBossBullet(self):
        global tick_immune_after_shield_boss, shield_bonus_taken_tick
        boss_bullet_Rect = pygame.Rect( boss_bullet.position.x, boss_bullet.position.y, boss_bullet.image.get_width(), boss_bullet.image.get_height() )
        shield_as_bonus_rect = pygame.Rect( self.x, self.y, energy_shield_number_5.get_width(), energy_shield_number_5.get_height() )
        if shield_as_bonus_rect.colliderect(boss_bullet_Rect):
            boss_bullets_removed.append(boss_bullet)
...
class BulletEnemyShooting:
    ...
    def update(self):
        ...
        if (self.position[0] > display_width or self.position[0] < 0) or (self.position[1] < 0 or self.position[1] > display_height):
    ...
    def collisionBulletShootingEnemy(self, player_one_istance, player_image):
        global shield_as_bonus_taken
        player_rect = pygame.Rect(player_one_istance.x, player_one_istance.y, player_image.get_width(), player_image.get_height() )
        #check collision
        if player_rect.colliderect(self.rect):
            enemy_bullets_list_removed.append(self)
...
class Boss:
    ...
    def __init__(self):
        ...
        self.life = 100
        self.tick_random_shoot = 0
        self.timer_damage = 0
        self.bool_damage = False
        self.angle = 0
        self.tick_to_shoot_for_boss = 0
        self.bool_random_shoot = False

    #calculate the angle for the bullet of the boss
    def rotate(self):
        temp_x, temp_y = player_one_istance.x - self.x, player_one_istance.y - self.y
        self.angle = (180 / math.pi) * -math.atan2(temp_y, temp_x) - 90

    def draw(self):
        self.tick_to_shoot_for_boss += 1
        if self.bool_damage:
            self.timer_damage += 1
    ...
    def checkIfDefeat(self):
        global score, fighting_boss, boss_present, boss_spawned, change_level_bool, change_level_running
        if boss_present and self.life <= 0:
            score += 500
            boss_present = False
            fighting_boss = False
            boss_spawned = False
            boss_defeated = True
            boss_list_removed.append(bosss)
            change_level_bool = True
            change_level_running = True
...
class BulletBoss:
    ...
    #check if the enemy shooting bullet collides with player and in that case decrement the life
    def collisionBulletBossWithPlayer(self, player_one_istance, player_image):
        global shield_as_bonus_taken
        player_rect = pygame.Rect(player_one_istance.x, player_one_istance.y, player_image.get_width(), player_image.get_height() )
        if player_rect.colliderect(self.rect):   #check collision
            boss_bullets_removed.append(self)
            if not shield_as_bonus_taken:
                player_one_istance.life -= 1
    ...
    def move_and_draw_stars(self):
        ...
        for star in self.stars:
            ...
            if 0 <= x < screen.get_width() and 0 <= y < screen.get_height():
                ...
                screen.fill((shade,shade,shade),(x,y,size,size))
...
start_ticks_to_reload_energy = pygame.time.get_ticks() #starter tick
start_ticks_to_keep_energy = pygame.time.get_ticks() #starter tick
seconds_to_reload = 0
seconds_to_keep = 0
#main loop
while running:
    ...
    if not laser_bool and player_one_istance.energy < 6:
        seconds_to_reload = (pygame.time.get_ticks() - start_ticks_to_reload_energy) / 1000  # calculate how many seconds
        if seconds_to_reload > 5:
            seconds_to_reload = 0
            start_ticks_to_reload_energy = pygame.time.get_ticks()
            player_one_istance.energy += 1

    if laser_bool and player_one_istance.energy > 0:
        seconds_to_keep = (pygame.time.get_ticks() - start_ticks_to_keep_energy) / 1000  # calculate how many seconds
        if seconds_to_keep > 2:
            seconds_to_keep = 0
            start_ticks_to_keep_energy = pygame.time.get_ticks()
            player_one_istance.energy -= 1
    ...
    if not boss_present and (len(shooting_enemies_list) == 0 or shooting_enemies_list[-1].y > 70) and not change_level_bool:
        shooting_enemies_list.append( ShootingEnemy() )
    ...
    if not boss_present and ((len(kamikaze_enemy_list) == 0 or kamikaze_enemy_list[-1].pos.y > 150)) and not change_level_bool:
    ...
    if boss_present and not boss_spawned:
    ...
    if boss_present:
        ...
        for bosss in boss_list:
            ...
            for boss_bullet in boss_bullets_list:
                boss_bullet.collisionBulletBossWithPlayer(player_one_istance, first_player_spaceship_image)
            for bullet2 in bullets_list:
                bullet2.collisionPlayerBulletWithBoss(bosss)
            if not bosss.bool_random_shoot and (bosss.life != 100 and bosss.life % 50 == 0):
                bosss.bool_random_shoot = True
            if bosss.bool_random_shoot == True:
                bosss.tick_random_shoot += 1
                if bosss.tick_random_shoot == 150:
                    bosss.bool_random_shoot = False
                    bosss.tick_random_shoot = 0
                else:
                    target = Vector2(random.uniform(0, display_width), random.uniform(0, display_height))
                    bullet_boss = BulletBoss(bosss.angle, (bosss.x, bosss.y), target)
                    boss_bullets_list.append(bullet_boss)
            else:
                if bosss.tick_to_shoot_for_boss != 0 and bosss.tick_to_shoot_for_boss % 10 == 0:
                    target = Vector2(player_one_istance.x, player_one_istance.y)
                    bullet_boss = BulletBoss(bosss.angle, (bosss.x, bosss.y), target)
                    boss_bullets_list.append(bullet_boss)
            bosss.checkIfDefeat()
    ...
    if shield_as_bonus_taken and not to_end_shield_animation:
        if tick_to_animate_shield == 5:
            ...
            for boss_bullet in boss_bullets_list:
                shield_as_shield_istance_one.collideBossBullet()
        else:
    ...
    if change_level_bool:
        bullets_list, shooting_enemies_list, enemy_bullets_list, kamikaze_enemy_list, boss_missile_list, boss_bullets_list, boss_list = [],[],[],[],[],[], []
        if not change_level_running:
            temp = Simulation(512, 32)
            change_level_running = True
        else:
            #temp.init_stars()
            screen.fill((27,32,38))
            temp.move_and_draw_stars()
            pygame.display.update()
